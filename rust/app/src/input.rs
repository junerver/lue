...
//! builds share one definition of every action; TTS-only keys decode to
//! `Action::Ignored` in the native build.
...
mod tests {
    ...
    fn default_layout_maps_core_keys() {
        ...
        assert_eq!(map.get("p"), Some(&Action::Ignored));
    }
