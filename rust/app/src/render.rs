...
fn draw_chapters(frame: &mut Frame, area: Rect, titles: &[String], filter: &str, selected: usize) {
    ...
    let items: Vec<ListItem> = visible
        .iter()
        .map(|&i| ListItem::new(titles[i].as_str()))
        .collect();
    let list = List::new(items)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(Span::styled(
                    format!(" Chapters (type to filter, Enter to jump): {filter} "),
...
mod tests {
    ...
    fn cycling_all_ui_modes_keeps_content_visible() {
        let dir = std::env::temp_dir().join("lue_render_tests");
