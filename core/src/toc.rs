...
/// True when a regex-matched line reads like a body sentence rather than a
/// chapter heading: real titles essentially always start with `第` and never
/// end with sentence-final punctuation. This is what keeps the loose keyword
/// branches of rules 0/1 (`正文(?!完|结)` etc., inherited from legado) from
/// promoting indented prose like "　　正文内容进入视线。" into a phantom
/// chapter between two real ones.
fn reads_like_prose(line: &str) -> bool {
    let trimmed = line.trim();
    !trimmed.starts_with('第') && trimmed.ends_with(|c: char| matches!(c, '。' | '！' | '？' | '…'))
}
...
pub fn collect_title_lines_impl(content: &str, rule_idx: usize) -> Vec<u64> {
    ...
    for m in re.find_iter(content).flatten() {
        ...
        let end = line_starts.get(line + 1).map_or(content.len(), |&s| s - 1);
        if reads_like_prose(&content[line_starts[line]..end]) {
            continue;
        }
...
mod tests {
    ...
    /// Reproduces the 魅力.txt phantom chapter: rule 0 wins the selection
    /// sample on clean 第X章 titles, then its `正文(?!完|结)` branch also
    /// matches indented prose sitting between 第五百五十一章 and 第五百五十
    /// 二章. The prose line must not start a chapter.
    #[test]
    fn indented_prose_with_sentence_punct_is_not_a_title() {
        let content = "书名 魅力\n\n第五百五十一章 品牌化之路\n　　快速点入笔记。\n　　正文内容进入视线。\n第五百五十二章 有过几面之缘\n";
        assert_eq!(collect_title_lines_impl(content, 0), vec![2, 5]);
    }

    /// The filter must stay conservative: 第-titled lines survive sentence
    /// punctuation, and keyword-marker titles (正文/番外…) survive when they
    /// do not read like a finished sentence.
    #[test]
    fn real_titles_are_kept() {
        let question = "开头说明\n第3章 你是谁？\n本章的正文如下。\n";
        assert_eq!(collect_title_lines_impl(question, 0), vec![1]);
        let keyword = "开头说明\n正文 第一卷 风起\n　　风起的内容。\n";
        assert_eq!(collect_title_lines_impl(keyword, 0), vec![1]);
    }
}
